...
int var_i = 0;
pilha_int pilha_var_i;

int tipo_op = 0;
pilha_int pilha_tipo_op;
...
programa:
   ...
   PROGRAM IDENT
   ABRE_PARENTESES lista_identificadores FECHA_PARENTESES PONTO_E_VIRGULA
   bloco PONTO
{
   char comando[100];
   int n_vars = quantidadeVars(tabela, nivel_lexico);
   if (n_vars > 0) {
   ...
   printf("Pilha var_i %d\n", tamanho_pilha_int(pilha_var_i));
   printf("Pilha tipo_op %d\n", tamanho_pilha_int(pilha_tipo_op));
   ...
   declaracao_procedure
{
   ...
   int sub_rotinas = quantidadeSubrotinas(tabela, nivel_lexico);
...
{
   char comando[100];
   int num_vars = quantidadeVars(tabela, nivel_lexico);
...
lista_identificadores:
   lista_identificadores VIRGULA IDENT
   | IDENT
;
...
write:
   WRITE ABRE_PARENTESES {tipo_op = PADRAO;} lista_write FECHA_PARENTESES 
...
secao_parametros_formais:
   lista_var_formais
   | FUNCTION lista_identificadores_formais DOIS_PONTOS IDENT
   | PROCEDURE lista_identificadores_formais
   |
;

lista_var_formais:
   VAR {empilha_int(REFERENCIA, &pilha_tipos);} lista_identificadores_formais DOIS_PONTOS IDENT
{
   if (strcmp(token, "integer") == 0)
   ...
   | {empilha_int(VALOR, &pilha_tipos);} lista_identificadores_formais DOIS_PONTOS IDENT
{
   if (strcmp(token, "integer") == 0)
      atualizaTipoParam(&tabela, INT, num_vars_tipo);
   else if (strcmp(token, "boolean") == 0)
...
lista_identificadores_formais:
   lista_identificadores_formais VIRGULA IDENT
{
...
chamada_sub_rotina:
{
   empilha_int(tipo_op, &pilha_tipo_op);
   tipo_op = SUB_ROT;

   empilha_int(var_i, &pilha_var_i);
   var_i = 0;
   ...
   chamada_parametros
{
   ...
   var_i = desempilha_int(&pilha_var_i);
   tipo_op = desempilha_int(&pilha_tipo_op);
...
expressoes:
   expressoes VIRGULA expressao 
{
   int tipo = desempilha_int(&pilha_tipos);
   if (sub_rotina.passagem_param[var_i] == REFERENCIA) {
      if (tipo_fator != F_IDENTIFICADOR || quant_fator > 1)
         imprimeErro ("Parametro invalido.");
   }

   if (tipo != sub_rotina.tipo_param[var_i])
      imprimeErro("Parametro invalido.");
...
operacao_booleana:
   ...
   | MENORIG { empilha_str("CMEG", &pilha_simbolos); }
   | MAIORIG { empilha_str("CMAG", &pilha_simbolos); }
;
   ...
   IDENT 
{
   ...
   quant_fator++;
   if (var.forma == FUNCAO) {
      if ((tipo_op == SUB_ROT) && (sub_rotina.passagem_param[var_i] == REFERENCIA))
      ...
      empilhaIDENT(token, var_i, quant_fator, tipo_op, sub_rotina, &pilha_tipos, tabela);
      tipo_fator = F_IDENTIFICADOR;
...
int main(int argc, char **argv)
{
   ...
   * ------------------------------------------------------------------- */
   ...
   inicializa_pilha_int(&pilha_var_i);
   inicializa_pilha_int(&pilha_tipo_op);
