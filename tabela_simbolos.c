...
int quantidadeVars (tabela_simbolos_t ts, int nivel) {
    for (int i = ts.topo; i >= 0; i--) {
...
int quantidadeSubrotinas(tabela_simbolos_t ts, int nivel) {
    int cont = 0;
...
void empilhaIDENT(char *token, int var_i, int quantFator, int tipo_op, sub_rotina_t rotina, pilha_int *pilha, tabela_simbolos_t ts) {
    char comando[100];
    simbolo_t simbolo = buscaSimbolo(ts, token);

    if (tipo_op == SUB_ROT) {
        if (var_i >= rotina.num_param)
            imprimeErro ("Quantidade de parâmetros inválido.");

        if (rotina.passagem_param[var_i] == REFERENCIA) {
            if (quantFator > 1)
